...
parser.add_argument("--cv_folds", type=int, default=5, help="Number of CV folds when feasible.")
parser.add_argument("--cv_repeats", type=int, default=10, help="Repeats for RepeatedStratifiedKFold.")
parser.add_argument("--shuffle_splits", type=int, default=20, help="Splits for StratifiedShuffleSplit fallback.")
parser.add_argument("--test_size", type=float, default=0.2, help="Test size for StratifiedShuffleSplit fallback.")
parser.add_argument("--permute_n", type=int, default=0, help="Number of label permutations per layer (0 disables).")
parser.add_argument("--seed", type=int, default=42, help="Random seed for CV + permutation.")
...
def permutation_baseline(X, y_int, seed, k_folds, n_perm):
    ...
    for i in range(n_perm):
        ...
        _, _, m_bal, _, _ = eval_probe_both(X, y_perm, seed=seed + 1000 + i, k_folds=k_folds)
...
print(f"Skipped: no_pred={skipped_no_pred}, no_activation={skipped_no_act}, bad_label={skipped_bad_label}")
...
for layer_idx in range(num_layers):
    ...
    # --- ALL usable ---
    X_all = np.stack([activations[r["id"]][layer_idx].float().numpy() for r in usable], axis=0)
    ...
    all_acc_mean.append(m_acc); all_acc_std.append(s_acc)
    all_bal_mean.append(m_bal); all_bal_std.append(s_bal)
    cv_used_all = cv_used_all or cv_name_all

    p_m, p_s = permutation_baseline(
        X_all, y_all, seed=args.seed + 20_000 + layer_idx,
        k_folds=args.cv_folds, n_perm=args.permute_n,
    )
    all_perm_bal_mean.append(p_m); all_perm_bal_std.append(p_s)

    # --- INCORRECT only ---
    if len(incorrect) > 0:
        X_inc = np.stack([activations[r["id"]][layer_idx].float().numpy() for r in incorrect], axis=0)
        ...
        if len(np.unique(y_inc)) >= 2:
            ...
            inc_acc_mean.append(m_acc_i); inc_acc_std.append(s_acc_i)
            inc_bal_mean.append(m_bal_i); inc_bal_std.append(s_bal_i)
            cv_used_inc = cv_used_inc or cv_name_inc

            p_mi, p_si = permutation_baseline(
                X_inc, y_inc, seed=args.seed + 50_000 + layer_idx,
                k_folds=args.cv_folds, n_perm=args.permute_n,
            )
            inc_perm_bal_mean.append(p_mi); inc_perm_bal_std.append(p_si)
        else:
            inc_acc_mean.append(None); inc_acc_std.append(None)
            inc_bal_mean.append(None); inc_bal_std.append(None)
            inc_perm_bal_mean.append(None); inc_perm_bal_std.append(None)
    else:
        inc_acc_mean.append(None); inc_acc_std.append(None)
        inc_bal_mean.append(None); inc_bal_std.append(None)
        inc_perm_bal_mean.append(None); inc_perm_bal_std.append(None)
...
if any(v is not None for v in inc_bal_mean):
    ...
    print(f"Best probe (INC) balanced acc: {best_inc_bal*100:.1f}% at layer {best_inc_layer}")
...
ax1.fill_between(layers, all_bal_m - all_bal_s, all_bal_m + all_bal_s, color="steelblue", alpha=0.2, linewidth=0)
if args.permute_n > 0 and not np.all(np.isnan(all_perm_m)):
    ax1.plot(layers, all_perm_m, color="black", linestyle="--", linewidth=1.0, label=f"Permuted labels (n={args.permute_n})")
    ax1.fill_between(layers, all_perm_m - all_perm_s, all_perm_m + all_perm_s, color="black", alpha=0.1, linewidth=0)
ax1.axhline(y=model_accuracy * 100, color="red", linestyle="--", label=f"Model output acc ({model_accuracy*100:.1f}%)")
ax1.axhline(y=chance_bal_acc * 100, color="gray", linestyle=":", label=f"Chance ({chance_bal_acc*100:.1f}%)")
ax1.set_xlabel("Layer"); ax1.set_ylabel("Balanced Accuracy (%)")
ax1.set_title("All Usable Stories"); ax1.legend(); ax1.grid(True, alpha=0.3)

valid_mask = ~np.isnan(inc_bal_m)
ax2.plot(layers[valid_mask], inc_bal_m[valid_mask], marker="o", color="darkorange", label="Probe (balanced acc, incorrect only)")
ax2.fill_between(layers[valid_mask], (inc_bal_m - inc_bal_s)[valid_mask], (inc_bal_m + inc_bal_s)[valid_mask], color="darkorange", alpha=0.2, linewidth=0)
if args.permute_n > 0 and not np.all(np.isnan(inc_perm_m)):
    ax2.plot(layers[valid_mask], inc_perm_m[valid_mask], color="black", linestyle="--", linewidth=1.0, label="Permuted labels")
    ax2.fill_between(layers[valid_mask], (inc_perm_m - inc_perm_s)[valid_mask], (inc_perm_m + inc_perm_s)[valid_mask], color="black", alpha=0.1, linewidth=0)
ax2.axhline(y=chance_bal_acc * 100, color="gray", linestyle=":", label=f"Chance ({chance_bal_acc*100:.1f}%)")
ax2.set_xlabel("Layer"); ax2.set_ylabel("Balanced Accuracy (%)")
